    ...
    <el-card>
      ...
      <el-table :data="tableData" v-loading="loading">
        ...
        <el-table-column prop="location" label="位置" width="100">
          <template #default="{ row }"><el-tag>{{ row.location === 'home' ? '首页' : '动态页' }}</el-tag></template>
        </el-table-column>
      ...
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="位置">
          <el-select v-model="form.location">
            ...
            <el-option label="动态页" value="video" />
          </el-select>
        </el-form-item>
...
const form = ref({ id: 0, image: '', title: '', linkUrl: '', location: 'home', sortOrder: 0, status: 1 })

const loadData = async () => {
  loading.value = true
  const res = await request.get('/api/v1/admin/content/banners')
  tableData.value = res.data || []
  loading.value = false
}

const handleAdd = () => { isEdit.value = false; Object.assign(form.value, { id: 0, image: '', title: '', linkUrl: '', location: 'home', sortOrder: 0, status: 1 }); dialogVisible.value = true }
const handleEdit = (row: any) => { isEdit.value = true; Object.assign(form.value, row); dialogVisible.value = true }

const handleSubmit = async () => {
  if (isEdit.value) {
    await request.put(`/api/v1/admin/content/banners/${form.value.id}`, form.value)
  } else {
    await request.post('/api/v1/admin/content/banners', form.value)
  }
...
const handleDelete = async (id: number) => {
  await request.delete(`/api/v1/admin/content/banners/${id}`)
